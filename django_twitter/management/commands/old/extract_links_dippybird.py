...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--domain_whitelist_only", action="store_true", default=False
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        if not options["use_multiprocessing"]:
            ...
            for chunk in tqdm(
                chunk_list(
                    list(tweets.values_list("pk", flat=True)),
                    1000 * options["num_cores"],
                ),
                total=tweets.count() / (1000 * options["num_cores"]),
                ...
                for subchunk in chunk_list(chunk, 1000):
                    results.append(
                        func(
                            ...
                            [
                                subchunk,
                                options["extract_secondary_links"],
                                options["expand_urls"],
                            ],
                        )
                    )
...
def extract_links(tweet_ids, extract_secondary_links, expand_urls):
    ...
    for tweet in tweets:
        tweet.extract_links(
            extract_secondary_links=extract_secondary_links,
            save=True,
            process=expand_urls,
            refresh=True,
        )
