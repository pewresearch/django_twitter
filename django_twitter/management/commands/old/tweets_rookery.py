...
from rookery_journalism.models import (
    TwitterUser,
    TwitterTweet,
    TwitterList,
    TwitterSearch,
    ...
    def handle(self, *args, **options):
        def consumeTimeline(params):
            def putTweet(item):
                ...
                placeRecord = (
                    twitter.generateRecord("place", item)
                    if hasattr(item.place, "id")
                    else None
                )
            ...
            if "list" in params:
                ...
                print(twitterParams)
                for item in list(
                    twitter.Cursor(twitter.api.list_timeline, **twitterParams).items()
                ):
                ...
                print(twitterParams)
                for item in list(
                    twitter.Cursor(twitter.api.search, **twitterParams).items()
                ):
                    tTweet, tUser = putTweet(item)
                    if timelineDirection:
                        maxId = tTweet.tweet_id if tTweet.tweet_id > maxId else maxId
                    ...
                    for tList in TwitterList.objects.filter(
                        list_id__in=tUser.lists.values_list("list_id")
                    ):
                ...
                print(twitterParams)
                for item in list(
                    twitter.Cursor(twitter.api.user_timeline, **twitterParams).items()
                ):
                    tTweet, tUser = putTweet(item)
                    if timelineDirection:
                        maxId = tTweet.tweet_id if tTweet.tweet_id > maxId else maxId
            ...
            if timelineDirection:
                if (
                    timelineDirection == "since_id"
                    and params[timelineDirection] > 0
                    and maxId > 0
                ):
                    params[timelineDirection] = maxId
                    print(
                        "Consuming timeline {} {}".format(
                            timelineDirection, params[timelineDirection]
                        )
                    )
                    consumeTimeline(params)
                elif (
                    timelineDirection == "max_id"
                    and params[timelineDirection] > 1
                    and minId > 1
                ):
        ...
        if options["get"]:
            paramError = CommandError(
                "For searches, --search is required; for users, --user-screen-name or --user-id is required; for lists, at least --list-id, or both --slug and either --owner-id or --owner-screen-name, is required."
            )

            params = (
                {"search": TwitterSearch.objects.filter(name=options["search"]).first()}
                if options["search"]
                else {
                ...
                if options["user_id"]
                    ...
                    "user": TwitterUser.objects.filter(
                        screen_name=options["user_screen_name"]
                    ).first()
                }
                ...
                if options["list_id"]
                    ...
                    "list": TwitterList.objects.filter(
                        slug=options["slug"], owner__user_id=options["owner_id"]
                    ).first()
                }
            ...
            if options["since_id"]:
                params["since_id"] = (
                    ...
                    else TwitterTweet.objects.filter(lists=params["list"])
                    .order_by("-tweet_id")
                    .first()
                    .tweet_id
                    if "list" in params
                    else TwitterTweet.objects.filter(searches=params["search"])
                    .order_by("-tweet_id")
                    .first()
                    .tweet_id
                    if "search" in params
                    else TwitterTweet.objects.filter(user=params["user"])
                    .order_by("-tweet_id")
                    .first()
                    .tweet_id
                    if "user" in params
