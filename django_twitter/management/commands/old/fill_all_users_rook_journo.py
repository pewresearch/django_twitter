...
class Command(BaseCommand):
        ...
        for user in tqdm(all_users, total=len(all_users)):
            if not user.twitter_error:
                ...
                try:  # a lot of these accounts (around 20%) are suspended
                    ...
                    for tweet_object in list(
                        twitter.Cursor(
                            twitter.api.user_timeline, user_id=user.user_id
                        ).items()
                    ):
                        tTweet, tUser = self._putTweet(
                            twitter, tweet_object, follow_links=False
                        )
    ...
    def _putTweet(self, twitter, item, follow_links=False):
        ...
        placeRecord = (
            twitter.generateRecord("place", item) if hasattr(item.place, "id") else None
        )
