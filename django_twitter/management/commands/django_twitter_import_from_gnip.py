...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["tweet_set"]:
            ...
            self.tweet_set, created = tweet_set_model.objects.get_or_create(name=options["tweet_set"])
...
def extract_data(payload):
    ...
    for link in profile_data['links']:
        if link['href']:
            tweet_data['user']['entities']['urls'].append({'expanded_url': link['href']})
    tweet_data['user']['verified'] = profile_data['verified']
    ...
    try:
        if tweet_payload.get('id'):
            ...
            if tweet_payload.get('inReplyTo', ''):
                tweet_data['in_reply_to_status_id'] = tweet_payload['inReplyTo']['link'].split('/')[-1]
                tweet_data['in_reply_to_status_id_str'] = tweet_data['in_reply_to_status_id']
                tweet_data['in_reply_to_user_id_str'] = tweet_payload['inReplyTo']['link'].split('/')[-3]
            ...
            if tweet_payload.get('twitter_quoted_status', {}):
                tweet_data['quoted_status'] = extract_data(tweet_payload['twitter_quoted_status'])
