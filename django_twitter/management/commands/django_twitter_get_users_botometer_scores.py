...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # setup models
        twitter_profile_model = apps.get_model(
        ...
        for twitter_id in tqdm(twitter_ids, total=len(twitter_ids)):
            try:
                user, created = twitter_profile_model.objects.get_or_create(
                ...
                user = twitter_profile_model.objects.filter(
                    twitter_id=twitter_id
...
def _get_botometer_score(twitter_id, key):
    call_command(
        "django_twitter_get_user_botometer_score", twitter_id, botometer_key=key
    )
