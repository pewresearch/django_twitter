    ...
    def handle(self, *args, **options):
        ...
        if options["from_twitter_profile_set"]:
            ...
            twitter_profile_set, created = twitter_profile_set_model.objects.get_or_create(
                name=options["from_twitter_profile_set"]
            )
            twitter_ids = list(
                twitter_profile_set.profiles.all().values_list("twitter_id", flat=True)
            )
        else:
        ...
        if not options["use_multiprocessing"]:
            ...
            for twitter_id in twitter_ids:
                if options["num_cores"] == 1:
                    pool.apply(
                        _collect_user_tweets,
                        args=(
                            twitter_id,
                            options["assign_twitter_profile_set"],
                            options["ignore_backfill"],
                            options["overwrite"],
...
def _collect_user_tweets(
    twitter_id,
    assign_twitter_profile_set,
    ignore_backfill,
    overwrite,
    max_backfill_date,
):
    call_command(
        "django_twitter_get_user",
        twitter_id,
    ...
    call_command(
        ...
        ignore_backfill=ignore_backfill,
        overwrite=overwrite,
        max_backfill_date=max_backfill_date,
        twitter_profile_set=assign_twitter_profile_set,
    )
