...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if twitter_json:
            ...
            try:
                following, created = user_model.objects.get_or_create(
                ...
                following = user_model.objects.filter(
                    twitter_id=twitter_json.id_str
            ...
            try:
                ...
                for follower_data in tqdm(
                    self.twitter.iterate_user_followers(
                        following.twitter_id, hydrate_users=options["hydrate"]
                    ),
                    ...
                    if not options["hydrate"]:
                        try:
                            follower, created = user_model.objects.get_or_create(
                                twitter_id=follower_data
                            )
                        except user_model.MultipleObjectsReturned:
                            print(
                            ...
                            follower = user_model.objects.filter(
                                twitter_id=follower_data
                            ).order_by("-last_update_time")[0]
                    else:
                        try:
                            follower, created = user_model.objects.get_or_create(
                                twitter_id=follower_data._json["id_str"]
                            )
                        except user_model.MultipleObjectsReturned:
                            print(
                                "Warning: multiple users found for {}".format(
