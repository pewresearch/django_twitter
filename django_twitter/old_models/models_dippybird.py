...
#
#
#     # BELOW: old code that was used to test out storing JSON in S3, but I/O was too slow
#
#     # def save_json(self, data, parse=True):
#     #
#     #     self.json.save(
#     #         "{}.json".format(self.tw_id),
#     #         ContentFile(json.dumps(data, indent=2, sort_keys=True))
#     #     )
#     #     if parse:
...
#     # def get_json(self):
#     #
#     #     return json.loads(self.json.read())
#
#     # def parse_json(self, data=None):
#     #
#     #     if not data:
#     #         data = self.get_json()
#     #     self.user = TwitterUser.objects.create_or_update({"user_id": data["user"]["id"]})
#     #     if not self.user.json:
#     #         self.user.save_json(data["user"], parse=True)
#     #     self.date_posted = parse_date(data["created_at"])
...
#     #     self.possibly_sensitive = data.get("possibly_sensitive", None)
#     #     self.is_reply = True if any([
