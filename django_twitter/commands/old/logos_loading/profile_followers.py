...
from logos.models import TwitterProfile, TwitterFollow, Date, Politician
        ...
        for profile_data in self.api.iterate_user_friends(source_profile.twitter_id, hydrate_users=self.options["hydrate_users"]):
            ...
            for profile in profiles:
                ...
                follow = TwitterFollow.objects.create_or_update(
                    {"follower": profile, "friend": source_profile}
                )
            ...
            if counter % 10 == 0:
                print "{} follows {} ({} active followers total)".format(source_profile, profile, counter)
